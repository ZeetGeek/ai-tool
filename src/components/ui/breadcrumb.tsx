...
const BreadcrumbList = React.forwardRef<HTMLOListElement, React.ComponentPropsWithoutRef<"ol">>(({ className, ...props }, ref) => (
	<ol
		ref={ref}
		className={cn("flex flex-wrap items-center gap-1.5 break-words text-sm text-neutral-500 dark:text-neutral-400 sm:gap-2.5", className)}
		{...props}
	/>
));
...
>(({ asChild, className, ...props }, ref) => {
	...
	return (
		<Comp
			ref={ref}
			className={cn("text-base capitalize text-neutral-400 transition-colors hover:text-neutral-950 dark:hover:text-neutral-50", className)}
			{...props}
		/>
	);
});
...
const BreadcrumbPage = React.forwardRef<HTMLSpanElement, React.ComponentPropsWithoutRef<"span">>(({ className, ...props }, ref) => (
	<span
		...
		aria-disabled="true"
		aria-current="page"
		className={cn("text-base font-semibold capitalize text-neutral-800 dark:text-neutral-50", className)}
		{...props}
	/>
));
